...
const accessSheet = async () => {
    ...
    await sheet.getRows({ limit: 24 }).then(rows => {
        rows.forEach((row) => {
            const gradeAverage = config.studentGradeAverage(parseInt(row.P1), parseInt(row.P2), parseInt(row.P3));
            ...
            Nota para aprovação final: ${(row.Nota_para_Aprovação_Final / 10).toFixed(0)}`);
        })
    })
}
accessSheet()
